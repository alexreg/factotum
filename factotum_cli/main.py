...
def load_factum(ctx: click.Context) -> Optional[Factum]:
    ...
    if module is not None:
        if verbose:  # pragma: no cover
            secho(f"Loading Factum from module `{module}`...", fg=colors.GREEN)
        ...
        if verbose:  # pragma: no cover
            secho(f"Loading Factum from path `{path}`...", fg=colors.GREEN)
    ...
    if spec is None:
        ...
        if module is not None:
            echo(f"Could not import `{module}` as Python module.", err=True)
        else:
    ...
    module_obj.__entry_point__ = True  # type: ignore
    ...
    if not hasattr(module_obj, "factum"):
        if hasattr(module_obj, "create_factum"):
            module_obj.factum = module_obj.create_factum()  # type: ignore
        else:
            if ctx.resilient_parsing:
                return None  # pragma: no cover

            echo(
                f"Module does not have `factum` or `create_factum` attribute.", err=True
            )
            raise Exit(1)
...
def docs(
    ...
    if factum is None:
        echo(f"No Factum found.", err=True)
        raise Exit(1)
...
def run(*args: Any, **kwargs: Any) -> Any:
    ...
    try:
        try:
            exit_code = app(standalone_mode=True, *args, **kwargs)
