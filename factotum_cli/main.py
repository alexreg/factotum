...
class AppGroup(TyperGroup):
    ...
    def maybe_add_run(self, ctx: click.Context) -> None:
        ...
        if path_param or module_param:
            factum = get_factum(ctx)
            if factum:
...
def get_factum(ctx: click.Context) -> Optional[Factum]:
    import importlib.machinery
    import importlib.util

    root_ctx = ctx.find_root()
    path_param: Path = root_ctx.params["path"]
    module_param: str = root_ctx.params["module"]

    spec = None
    if module_param:
        spec = importlib.util.find_spec(module_param)  # type: ignore
    else:
        path_param = path_param or Path("Factum")
        module_name = path_param.name
        spec = importlib.util.spec_from_loader(
            module_name,
            importlib.machinery.SourceFileLoader(module_name, str(path_param)),
        )

    if spec is None:
        if module_param:
            typer.echo(f"Could not import as Python module: {module_param}", err=True)
        else:
    ...
    factotum.main.script_path = Path(module.__file__) if module.__file__ else None

    spec.loader.exec_module(module)

    cli: Optional[Factum] = getattr(module, "cli", None)
    if cli is None:
        return None

    cli._add_completion = False
...
def callback(
    ...
    *,
    ...
    # TODO: make this actually output useful info
