...
def test_script_help():
    ...
    assert "factotum_cli --path tests/assets/add_sub_path.py run" in result.stdout
    assert "Imports sample factum" in result.stdout
...
def test_script_imported_help():
    ...
    assert result.returncode == 0
    assert (
        "factotum_cli --path tests/assets/add_sub_path.py run imported" in result.stdout
    )
